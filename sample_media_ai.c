...
#define HAND_FRM_WIDTH    384
#define HAND_FRM_HEIGHT   216
    ...
    int grpId, const VPSS_GRP_ATTR_S* grpAttr, int maxWidth, int maxHeight)
{
    ...
    if (grpAttr) {
        ...
        self->grpAttr.stNrAttr.enNrType = VPSS_NR_TYPE_VIDEO;
        self->grpAttr.stNrAttr.enNrMotionMode = NR_MOTION_MODE_NORMAL;
        self->grpAttr.stNrAttr.enCompressMode = COMPRESS_MODE_FRAME;
    }
}
...
int ViVpssCreate(MppSess** sess, const ViCfg* viCfg, const VpssCfg* vpssCfg)
{
  ...
  /*   stLowDelayInfo.bEnable = HI_TRUE;
    stLowDelayInfo.u32LineCnt = 480/2;
    ret = HI_MPI_VPSS_SetLowDelayAttr(vpssCfg->grpId,1,&stLowDelayInfo); */
    //--------------
...
static HI_VOID HandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        ...
        ret = MppFrmResize(&frm, &resizeFrm, HAND_FRM_WIDTH, HAND_FRM_HEIGHT);//缩放我们之前取到的一帧图片
        ret = Yolo2HandDetectResnetClassifyCal(g_workPlug.model, &resizeFrm, &frm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,"hand classify plug cal FAIL, ret=%#x\n", ret);
...
static HI_VOID* GetVpssChnFrameHandClassify(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    VIDEO_FRAME_INFO_S frm1;
    HI_S32 s32MilliSec = 2000;
    VO_LAYER voLayer = 0;
    VO_CHN voChn = 0;
    time_t start, end;
    double totalTime;
    double elapsed; // 用于存储经过的时间
    SAMPLE_PRT("vpssGrp:%d, vpssChn0:%d\n", g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0);

    while (HI_FALSE == g_bAiProcessStopSignal) {
           ret = HI_MPI_VPSS_GetChnFrame(g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0, &frm, s32MilliSec);//得到一帧图片。
           //ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm1, 0);
         if (ret != 0) {
            SAMPLE_PRT("HI_MPI_VPSS_GetChnFrame FAIL, err=%#x, grp=%d, chn=%d\n",
                ret, g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0);
            ret = HI_MPI_VPSS_ReleaseChnFrame(g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0, &frm);
...
/*          ret = HI_MPI_VPSS_GetChnFrame(g_aicMediaInfo.vpssGrp,0, &frm1, s32MilliSec);//得到一帧图片。
          if (ret != 0) {
            SAMPLE_PRT("HI_MPI_VPSS_GetChnFrame FAIL, err=%#x, grp=%d, chn=%d\n",
                ret, g_aicMediaInfo.vpssGrp,0);
            ret = HI_MPI_VPSS_ReleaseChnFrame(g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0, &frm);
            if (ret != HI_SUCCESS) {
                SAMPLE_PRT("Error(%#x),HI_MPI_VPSS_ReleaseChnFrame failed,Grp(%d) chn(%d)!\n",
                    ret, g_aicMediaInfo.vpssGrp, g_aicMediaInfo.vpssChn0);
...
//-------
    u64BlkSize_venc = COMMON_GetPicBufferSize(g_aicMediaInfo.stSize.u32Width, g_aicMediaInfo.stSize.u32Height,PIXEL_FORMAT_YVU_SEMIPLANAR_422, DATA_BITWIDTH_8, COMPRESS_MODE_SEG,DEFAULT_ALIGN);
    self->astCommPool[1].u64BlkSize   =  g_aicMediaInfo.u32BlkSize;
    self->astCommPool[1].u32BlkCnt    = 16; 
//-------
    /*
     * 获取raw buffer的大小
     * Get raw buffer size
  ...
  /*   g_aicMediaInfo.u32BlkSize = VI_GetRawBufferSize(g_aicMediaInfo.stSize.u32Width, g_aicMediaInfo.stSize.u32Height,
    ...
    self->astCommPool[1].u32BlkCnt   = 5; */
}
...
static HI_VOID VpssParamCfg(HI_VOID)
{
    ...
    //---------
    VpssCfgAddChn(&g_aicMediaInfo.vpssCfg, 2, NULL,AICSTART_VI_OUTWIDTH,AICSTART_VI_OUTHEIGHT);
    //----------
...
//------------------------------------------------------------------
    ...
    if (HI_SUCCESS != s32Ret)
    {
     ...
     /***encode h.264 **/
    s32Ret = SAMPLE_COMM_VENC_Start(0,PT_H264,PIC_D1_MIPI, SAMPLE_RC_CBR,0,HI_TRUE,&stGopAttr);
    //创建venc编码通道并且开启编码
    if (HI_SUCCESS != s32Ret)
    {
  ...
  /*   s32Ret = SAMPLE_COMM_VPSS_Bind_VENC(0,2,0);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Venc bind Vpss failed for %#x!\n", s32Ret);
    }  */
    s32Ret = SAMPLE_COMM_VENC_StartGetStream(VencChn,1);//开启一个线程进行从编码通道里获取编码流给环形缓冲区。
    if (HI_SUCCESS != s32Ret)
    {
